...
import clientesRepository from '../api/repository/clientesRepository';
dotenv.config();

beforeAll(async () => {
  await app.listen(); // Inicia o servidor
});

let id: number;
const url: string = `http://localhost:${process.env.PORTA}/clientes`;
...
describe('updateCliente', () => {
  it('deve atualizar um cliente', async () => {
    ...
    const response = await axios.put(`${url}/${id}`, clienteAtualizado);
    expect(response.data.nome).toBe('Cliente Atualizado');
    expect(response.data.email).toBe('atualizado@atualizado.com');
    expect(response.data.telefone).toBe('123456789');
    expect(response.data.coordenada_x).toBe(12.34);
    expect(response.data.coordenada_y).toBe(56.78);
    expect(response.status).toBe(200);
  });
});
